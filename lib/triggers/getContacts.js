...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function getDeletedContacts(instance, deltaToken){
    ...
    let url = '/me/contacts/delta';
    if (deltaToken) url = url += `?${deltaToken}`;
    console.log('About to get delta with url: ', url);
  ...
  async function getAllContacts(from, until) {
    ...
    */
    ...
    if (cfg.deletes) {
      ...
      const { deletedContacts, nextDeltaToken} = await getDeletedContacts(instance, snapshot.deltaToken)
