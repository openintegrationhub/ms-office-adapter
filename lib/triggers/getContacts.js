...
async function getContacts(instance, from, until, folderId){
  ...
  const filter = encodeURIComponent(`start/dateTime ge '${from}' and end/dateTime le '${until}`);

  let result = await apiClient.get(`${actionUrl}?$filter=${filter}&$top=100&$skip=0`);
  ...
  while('@odata.nextLink' in result){
    c += 100;
    result = await apiClient.get(`/me/contacts?$filter=${filter}&$top=100&$skip=${c}`);
    if('value' in result) {
      contacts = contacts.concat(result.value);
    }
  }
...
async function getAllContacts(from, until) {
  let contacts = await getBaseContacts(instance, from, until);
...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function emitData() {
    ...
    */
    ...
    const contacts = getAllContacts(snapshot.lastUpdated, Date.now());
