...
module.exports.contactDataFromOih = function(entry) {

  const oldEntry = false;
  ...
  if('contactData' in entry){
    for(index in entry.contactData) {
      if(
        !entry.contactData[index].value
        || typeof entry.contactData[index].value !== 'string'
        || entry.contactData[index].value.trim() === ''
      ) continue;
      ...
      if(entry.contactData[index].type === 'email'){
        ...
        if(phoneType === 'home') {
          if(homePhones.length < 2) homePhones.push(entry.contactData[index].value);
        } else {
          businessPhones.push(entry.contactData[index].value);
        }
